  ...
  if(isset($_GET['mt']) && !empty($_GET['mt']))
  {
    ...
    if($monthval=='yr')
    {
       for($j=0;$j<count($taskcats);$j++){
         for ($i = 1; $i <= 12; $i++)
         {
            $mycount  = mysqli_query($convar->conn, "SELECT COUNT(TaskID) FROM taskdetails WHERE MONTH(TaskDate)=$i AND YEAR(TaskDate)=$curyr AND TaskCategory='$taskcats[$j]'");
    ...
    for($j=0;$j<count($taskcats);$j++){
      for ($i = 1; $i <= $numdays; $i++)
      {
        $mycount  = mysqli_query($convar->conn, "SELECT COUNT(TaskID) FROM taskdetails WHERE DAY(TaskDate)=$i AND MONTH(TaskDate)=$curmonth AND YEAR(TaskDate)=$curyr AND TaskCategory='$taskcats[$j]'");
        $mycountrow  = mysqli_fetch_row($mycount);
        if($mycountrow==null)
        {
            $mycountrow=0;
    ...
    if(chartcount[j][i]==0){
       ...
       Highcharts.chart('tasktrack', {
        ...
        yAxis: {
            ...
            allowDecimals: false,
            title: {
                text: 'Category-wise Task Breakdown'
            },
            stackLabels: {
                enabled: true,
                style: {
